...
public:
    ...
    */
    ...
    GenericValue(int32_t i) : flags_(kNumberIntFlag) { 
        data_.n.i64 = i;
        if (i >= 0)
    ...
    GenericValue(uint32_t u) : flags_(kNumberUintFlag) {
        data_.n.u64 = u; 
        if (!(u & 0x80000000))
            flags_ |= kIntFlag;
    ...
    bool RemoveMember(const Ch* name) {
        ...
        if (Member* m = FindMember(name)) {
            ...
            if ((data_.o.size > 1) && (m != last)) {
                ...
                // Only one left or last element, so destroy
                m->name.~GenericValue();
                m->value.~GenericValue();
            }
            --data_.o.size;
...
private:
    ...
    Member* FindMember(const Ch* name) {
        ...
        for (Member* member = o.members; member != data_.o.members + data_.o.size; ++member)
            ...
            if ((member->name.data_.s.length == name_length) && memcmp(member->name.data_.s.str, name, member->name.data_.s.length * sizeof(Ch)) == 0)
                return member;
...
public:
    ...
    */
    GenericDocument(Allocator* allocator = 0, size_t stackCapacity = kDefaultStackCapacity) : stack_(allocator, stackCapacity), parseError_(0), errorOffset_(0), line_number_(0) { }
    ...
    */
    ...
    GenericDocument& ParseStream(Stream& stream) {
        ...
        if (reader.Parse<parseFlags>(stream, *this)) {
            ...
            line_number_ = reader.GetLineNumber();
            ClearStack();
        }
        return *this;
    }
