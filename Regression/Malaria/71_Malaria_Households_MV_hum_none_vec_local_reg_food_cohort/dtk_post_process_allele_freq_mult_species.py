...
# a list of GenomeData objects representing each genome
# --------------------------------------------------------------
    ...
    with open( filename, "r" ) as rs:
        ...
        for line in rs:
            ...
            if len(line_array) == len(col_indexes_map):
                time   = line_array[ col_indexes_map["Time"] ]
                ...
                if( time != prev_time ):
                    ...
                    prev_time = time
                    num_time_steps += 1
                elif( len(data_map[ genome ]) < num_time_steps ):
                    data_map[ genome ].append( data )
                else:
                    data_map[ genome ][-1] += data
