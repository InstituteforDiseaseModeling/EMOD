...
Indoor_Biting: 0.7
Giving one person 100% Repelling UsageDependentBednet that they use half the time
0.7/2 + 0.3 / ( 2+0.7/2 + 0.3) = 0.245
the other two are 0.377 each 
"""
...
def application(output_folder="output",
    ...
    humans, human_bites, bites_by_day = veds.track_bites(stdout, start_time=2, debug=False)
    veds.create_biting_report_file(param_obj=param_obj, humans=humans, bites_by_day=bites_by_day,
                                   ...
                                   with_intervention_proportion=0.245,
                                   without_intervention_proportion=0.377,
