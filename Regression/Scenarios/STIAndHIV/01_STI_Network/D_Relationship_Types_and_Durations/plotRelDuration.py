...
NOTE: This script is using library "plotnine" to generate the graph.  
"""
...
def plot_relationship_duration(end_df, demo_object, file_dir="figs", fig_name="RelationshipDuration_python.png"):
        if not os.path.isdir(file_dir):
            os.mkdir(file_dir)

        #end_df[ReportHeader.type] = end_df[ReportHeader.rel_type].map(relationship_table)

        end_df[ReportHeader.rel_type] = end_df[ReportHeader.rel_type].astype('category')
        # print(end_df[ReportHeader.rel_type])
        g = (ggplot(data=end_df, mapping=aes(x=ReportHeader.scheduled_duration_year,
                                             group=ReportHeader.rel_type,
                                             fill=ReportHeader.rel_type)) +
             geom_density(alpha=0.3) +
             stat_function(
                 fun=lambda x: stats.weibull_min.pdf(x,
                                                     1/demo_object[relationship_table[0]][DemoPfaKeys.heterogeneity],
                                                     0,
                                                     demo_object[relationship_table[0]][DemoPfaKeys.scale]),
                 colour="red",
                 linetype="dashed") +
             stat_function(
                 fun=lambda x: stats.weibull_min.pdf(x,
                                                     1/demo_object[relationship_table[1]][DemoPfaKeys.heterogeneity],
                                                     0,
                                                     demo_object[relationship_table[1]][DemoPfaKeys.scale]),
                 colour="green",
                 linetype="dashed") +
             stat_function(
                 fun=lambda x: stats.weibull_min.pdf(x,
                                                     1/demo_object[relationship_table[2]][DemoPfaKeys.heterogeneity],
                                                     0,
                                                     demo_object[relationship_table[2]][DemoPfaKeys.scale]),
                 colour="blue",
                 linetype="dashed") +
             xlab("Duration (years)") +
             ylab("Density") +
             scale_y_continuous(limits=[0.0, 0.8]) +
             ggtitle("Relationship Duration by Type")  +
             scale_fill_manual(name="Type",
                            # breaks=['TRANSITORY', 'INFORMAL', 'MARITAL'],
                               breaks=[0, 1, 2],
                               labels=['Transitory', 'Informal', 'Marital'],
                               values=["red", "green", "#1E90FF"]))
...
def application(output_folder="output", report_filename="RelationshipEnd.csv",
                ...
                fig_name="RelationshipDuration_python.png"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--fig_name', default="RelationshipDuration_python.png",
                        help="filename for plot(RelationshipDuration_python.png)")
