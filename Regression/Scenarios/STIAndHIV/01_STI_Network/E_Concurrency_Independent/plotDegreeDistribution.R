...
degreeDistributionByRelType = ddply( degreeDistributionByRelType, .(Gender, Rel_Type, Scenario), transform, Total=sum(Count))
degreeDistributionByRelType$Percent = 100 * degreeDistributionByRelType$Count / degreeDistributionByRelType$Total

p = ggplot(degreeDistributionByRelType, aes(x=Degree, y=Percent, group=Gender, fill=Gender)) +
    geom_bar(position="dodge", stat="identity") +
    xlab( "Current Number of Relationships by Type" ) +
    ylab( "Percent" ) +
    ggtitle( "Degree Distribution by Relationship Type: Independent" ) +
...
degreeDistribution = ddply( degreeDistribution, .(Gender, Scenario), transform, Total=sum(Count))
degreeDistribution$Percent = 100 * degreeDistribution$Count / degreeDistribution$Total

p = ggplot(degreeDistribution, aes(x=Degree, y=Percent, group=Gender, fill=Gender)) +
    geom_bar(position="dodge", stat="identity") +
    xlab( "Current Number of Relationships" ) +
    ylab( "Percent" ) +
    ggtitle( "Degree Distribution: Independent" ) +
    scale_fill_manual(breaks=c("Male", "Female"), name="Gender", values=c("Blue", "Red")) +
