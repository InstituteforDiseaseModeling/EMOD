...
"""
This test is testing the relationship targeting: HasRelationship feature with With_Partner_Who and 
Of_Relationship_Type options.

The main test is making sure the MyMaritalPartnerInRiskHigh event is only distributed to who has Marital relationship and
Marital partner is in Risk:High
...
def create_report_file(end_df, start_df, event_df, end_report_name, start_report_name, event_report_name,
    ...
    with open(output_report_name, 'w') as output_report_file:
        ...
        start_df = start_df[start_df[rt_support.Constant.rel_type] == rt_support.Constant.marital_type]
        ...
        result, partner_df, target_df = rt_support.get_highriskpartner_and_target_df(
            rt_support.Event.my_marital_partner_in_risk_high,
        ...
        if succeed:
            if not rt_support.hasRelationship_test(partner_df, target_df, start_df, testing_options,
                                                   rt_support.Event.my_marital_partner_in_risk_high,
                                                   "Risk:High group in a Marital relationship",
                                                   output_report_file):
                succeed = False
...
def application( output_folder="output", stdout_filename="test.txt",
    ...
    testing_options = ["With_Partner_Who", "Of_Relationship_Type"]
