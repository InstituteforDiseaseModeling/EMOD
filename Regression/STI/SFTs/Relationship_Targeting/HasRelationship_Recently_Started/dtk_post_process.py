...
option set to STARTED.

The main test is making sure the MyRelationshipJustStarted event is only distributed to who has relationship that have 
started within one time-step and the partner in this relationship is in Risk:High group.
...
def create_report_file(end_df, start_df, event_df, end_report_name, start_report_name, event_report_name,
    ...
    with open(output_report_name, 'w') as output_report_file:
        ...
        end_df = end_df[(end_df[rt_support.Constant.rel_start_time] >= start_day - 1) &
                        (end_df[rt_support.Constant.rel_start_time] <= start_day) &
                        (end_df[rt_support.Constant.rel_actual_end_time] <= start_day)]
        # Keep only relationship that are recently started from start_df
        start_df = start_df[(start_df[rt_support.Constant.rel_start_time] == start_day - 1)]
        ...
        if start_df.empty:
            succeed = False
            output_report_file.write(f"BAD: There are no relationship that have started at time {start_day - 1}.\n")
        ...
        result, partner_df, target_df = rt_support.get_highriskpartner_and_target_df(
            rt_support.Event.my_relationship_just_started,
        ...
        if succeed:
            if not rt_support.hasRelationship_test(partner_df, target_df, start_df, testing_options,
                                                   rt_support.Event.my_relationship_just_started,
                                                   "Risk:High group in a relationship that have started within one time-step",
                                                   output_report_file):
                succeed = False
