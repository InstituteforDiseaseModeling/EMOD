...
"""
InterventionForCurrentPartners\OLDER_AGE
This test is testing the InterventionForCurrentPartners with Prioritize_Partners_By set to OLDER_AGE.
...
3. Load RelationshipStart.csv, collect relationships table for female id with her oldest partner.
4. load ReportEventRecorder.csv and:
    4.1: check number of broadcast event
    4.2: check if all female individuals' oldest partner received the exact number of event
    4.3: make sure no male individuals who should not receive the event did not receive any event
...
def create_report_file(output_folder, report_event_recorder, relationship_start_report, relationship_end_report,
    ...
    with open(output_report_name, 'w') as output_report_file:
        ...
        # collect relationships table for female id with her oldest partner
        relationships = {}  # relationships dictionary for female_id: male_id
        ...
        for index, row in start_df.iterrows():
            ...
            if f_id not in relationships:
                ...
                oldest_male = relationships[f_id]  # [0]
                oldest_male_age = age_map[oldest_male][1] + time - age_map[oldest_male][0]
                if age > oldest_male_age:
                    relationships[f_id] = m_id  # [m_id]
                elif age == oldest_male_age:
                    output_report_file.write(f"WARNING: male {m_id} and {oldest_male} are both the oldest male partner"
                                             f"for female {f_id} at day {time}. We assume intervention will be send"
                                             f" to {oldest_male}.\n")
        ...
        # check if all female individuals' oldest partner received the exact number of event
        ...
        if len(relationships):
            ...
            output_report_file.write(f"BAD: these male ids are expected to receive the event based on "
                                     f"{icp_s.Events.OLDER_AGE} priority but they didn't receive it or receive less "
            ...
            output_report_file.write(f"GOOD: all male ids that are expected to receive the event based on "
                                     f"{icp_s.Events.OLDER_AGE} priority have received the expected # of events.\n")
