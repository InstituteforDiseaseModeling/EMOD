...
#              The ramp durantions and cutoff days need to follow a rule where: ramp_up_duration + ramp_down
# duration + cutoff_days < 365
...
def application(report_file, debug=False):
    ...
    with open("test.txt") as logfile:
        for line in logfile:
            ...
            if re.search("Exposing", line) and re.search("environment", line):
                line = "TimeStep: " + str(timestep) + " " + line
                lines.append(line)
    with open("DEBUG_filtered_line.txt",'w') as filtered_file:
        filtered_file.write(''.join(lines))
    ...
    with open(sft.sft_output_filename, "w") as report_file:
        if count == 0:
            ...
            success = False
            report_file.write("BAD: Environmental peak duration should be larger or equal to 0, the actual "
                              "value is {}.\n The ramp durantions and cutoff days need to follow a rule where: "
                              "ramp_up_duration + ramp_down_duration + cutoff_days < 365.\n".format(peak_duration))
        else:
                ...
                if re.search("Update\(\): Time:", line):
                    ...
                    if math.fabs(amplification - environmental_amplification) > 5e-2:
                        ...
                        report_file.write("BAD: at time {0}, day of year = {1}, the environmental amplification is {2},"
                                          " expected {3}.\n".format(TimeStep, day_in_year+adjust_time,
                                                                    amplification, environmental_amplification))
        ...
        sft.plot_data_sorted(amp, environmental_amp,
                      ...
                      title="Seasonal Attenuation CD Equals 0", xlabel="Time",
                      ylabel="Attenuation",
                      category='seasonal_attenuation_cdequals0', overlap=True, alpha=0.5)
