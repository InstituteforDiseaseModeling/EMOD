...
def application( output_path ):
    ...
    with open( csv_fn, "r" ) as rs:
            ...
            for index in range(len(values)):
                ...
                if( name == "Time" ):
                    ...
                    if( prev_time != time ):
                        ...
                        for name in column_names:
                            if( (name != "Time") and (name != "NodeID") ):
                                json_data[ "Channels" ][ name ][ "Data"  ].append( 0 )
                elif( (name == "NodeID") and (time == start_time) ):
                    num_nodes += 1
                elif( name != "NodeID"):
        ...
        for name in column_names:
            if( (name != "Time") and (name != "NodeID") ):
                for i in range(len(json_data[ "Channels" ][ name ][ "Data"  ])):
                    json_data[ "Channels" ][ name ][ "Data"  ][i] = json_data[ "Channels" ][ name ][ "Data"  ][i] / num_nodes
