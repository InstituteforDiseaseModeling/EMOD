...
def GetColumnIndexesFromHeader( header ):
    ...
    used_columns.append( "VectorPopulationNumDied"       )
    used_columns.append( "InfectiousNumDied"             )
    used_columns.append( "InfectedNumDied"               )
    used_columns.append( "AdultNumDied"                  )
    used_columns.append( "MaleNumDied"                   )
    used_columns.append( "VectorPopulationAvgAgeAtDeath" )
    used_columns.append( "InfectiousAvgAgeAtDeath"       )
    used_columns.append( "InfectedAvgAgeAtDeath"         )
    used_columns.append( "AdultAvgAgeAtDeath"            )
    used_columns.append( "MaleAvgAgeAtDeath"             )
    ...
    with open( filename, "r" ) as rs:
        ...
        for line in rs:
            ...
            if len(line_array) == len(col_indexes_map):
                ...
                data += float(   line_array[ col_indexes_map["STATE_MALE"] ])
...
def application( output_path ):
    ...
    allele_list = []
    allele_list.append( "X1" )
    allele_list.append( "X2" )
    allele_list.append( "Y1" )
    allele_list.append( "Y2" )
    
    input_filename = os.path.join( output_path, "ReportVectorGenetics_SillySkeeter_GENOME.csv" )
