...
def create_report_file(config_filename, report_name, debug=False):
    with open(report_name, "w") as outfile:
        ...
        feed_time = 16
        # fem. vectors feed and get infected on day 1 which is counted as day 0 of infections, feed again on day 16
        time_from_disease = feed_time - 2
        ...
        total_bites = int(vector_stats_df.loc[(vector_stats_df['Time'] == feed_time), "IndoorBitesCount"].item())
        infection_bites = int(vector_stats_df.loc[(vector_stats_df['Time'] == feed_time), "IndoorBitesCount-Infectious"].item())
