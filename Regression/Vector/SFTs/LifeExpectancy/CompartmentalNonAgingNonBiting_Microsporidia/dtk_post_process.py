        ...
        if deaths_df.empty:
            ...
            for has_microsporidia in microsporidia:
                for sex in sexes:
                    ...
                    sub_df = sub_df[deaths_df.microsporidia == has_microsporidia]
                    ...
                    dtk_sft.plot_data(population_by_sex[sex], dist2=None, label1=f"{sex} Vectors of that Age",
                                      label2=None, title=f"{sex} {'microsporida' if has_microsporidia else ''} Vector Population Age Distribution",
                                      xlabel="Age (in days)", ylabel="Number of vectors",
                                      category=f'{sex}_vector_age_distribution_{"has" if has_microsporidia else "no"}_microsporidia')
                    ...
                    if not vles.check_results(mortality_check, f"{sex} {'has' if has_microsporidia else 'no'} microsporidia", success_rate, outfile):
                        success = False
                    life_expectancy = life_expectancy / mortality_modifier
                    ...
                    error = 0.6  # want them to be within half a day
