...
def create_report_file(param_obj, report_name, temperature_k, output_df, debug=False):
    ...
    with open(report_name, "w") as outfile:
        outfile.write("# Test name: " + param_obj[veds.ConfigKeys.CONFIG_NAME] + ", Run number: " +
                      ...
                      "\n# Test"
                      " compares between the actual and expected larval development durations to verify "
                      "larvae develop with an exponential distribution,\n# and the mean "
                      "of the development distribution is"
        ...
        if debug:
            print(param_obj)
        sft.format_success_msg(success)
...
def parse_output_file(stdout, debug=False):
    ...
    with open(stdout) as logfile:
        for line in logfile:
            ...
            if "larval queues" in line:
                ...
                population_duration[id_duration[str(identity)]] = 0
            if "Immature adults" in line:
                ...
                population_duration[id_duration[str(identity)]] += population
    ...
    d = {"populations": [value for value in population_duration.values() if value is not 0],
         "durations": [float(key) for key in population_duration.keys() if population_duration[key] is not 0]}
